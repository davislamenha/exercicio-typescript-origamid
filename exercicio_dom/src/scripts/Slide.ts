...
export default class Slide {
  ...
  constructor(
    ...
    this.time = time;
    this.index = localStorage.getItem('currentSlide')
      ? Number(localStorage.getItem('currentSlide'))
      : 0;
  ...
  show(index: number) {
    this.index = index;
    this.slide = this.slides[this.index];
    localStorage.setItem('currentSlide', `${this.index}`);
  ...
  autoVideo(video: HTMLVideoElement): void {
    ...
    video.addEventListener('playing', () => {
      if (firstPlay) this.auto(videoDuration);
      firstPlay = false;
    });
  }
  ...
  private addThumbItems() {
    ...
    for (let i = 0; i < this.slides.length; i++) {
      thumbContainer.innerHTML += `<span><span class='thumb_item'></span></span>`;
    }
